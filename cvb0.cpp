/* This file contains an implementation of the Zeroth Order Tayler approximation to the Collapsed Variational Bayes algorithm for Latent Dirichlet Allocation, referred to as CVB0.
 *
 ...
 */
double **InitPHIS(int *Ns, int M, int K) {
...
List TestPHIS() {
    ...
    Ns(1) = 2;
    int K = 3;
    double **PHIS = InitPHIS(Ns.begin(), M, K);
    List ret(0);
    NumericMatrix PHI;
    for (int m = 0; m < M; m++) {
        PHI = NumericMatrix(K, Ns(m), *(PHIS + m));
        ret.push_back(PHI);
    }
...
// [[Rcpp::export]]
NumericMatrix TestNwk(List docs_in, int K, int V, NumericVector eta, int seed) {
    //Turn the docs object into something consumeable by C and store some facts about it
    int M = docs_in.size();
    ...
    NumericMatrix Nwk = NumericMatrix(K, V, InitNwk(PHIS, docs, Ns, eta.begin(), M, K, V));
